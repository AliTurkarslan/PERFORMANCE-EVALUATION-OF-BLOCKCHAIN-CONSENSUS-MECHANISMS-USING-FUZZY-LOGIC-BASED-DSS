...
def collect_metrics(scenario, provider="http://localhost:8545"):
    """
    Collects benchmarking metrics from a PoA-based Ethereum network (Clique).
    Also logs per-transaction data to tx_detailed_log.csv.
    """
    ...
    for i, (tx_hash, sent_time_str) in enumerate(tx_data.items()):
        ...
        if receipt:
            ...
            latency = max(block_time - sent_time, 0)
            gas = receipt.gasUsed
            ...
            if receipt.blockNumber not in block_times:
                block_times[receipt.blockNumber] = block_time
            ...
            detailed_logs.append({
                ...
                "block_timestamp": datetime.utcfromtimestamp(block_time).isoformat(),
                "latency": latency,
                "gas_used": gas,
    ...
    print(f"✅ PoA metrics summary saved to: {metrics_output_path}")
    print(f"✅ PoA detailed log saved to: {detailed_log_path}")
    return summary

# CLI Entry Point
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Collect benchmark metrics for Ethereum PoA scenario")
    parser.add_argument("--scenario", type=str, required=True, help="Scenario name (e.g., poa_500tps)")
